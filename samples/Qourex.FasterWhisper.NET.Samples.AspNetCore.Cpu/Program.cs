...
app.MapGet("/", () => Results.Text("FasterWhisper.NET API is running! Post a WAV file to /api/transcribe to transcribe."));
...
public class WhisperService : IDisposable
{
    ...
    private async Task EnsureModelLoadedAsync()
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Loading model from path: {Path} on CPU", modelPath);
    ...
    public async Task<TranscriptionResponse> TranscribeAsync(string wavFilePath)
    {
        ...
        _logger.LogInformation("Running transcription...");
